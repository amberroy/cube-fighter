...
EXPLODE = function(scene, object3D) {
  ...
  if (object3D.geometry instanceof THREE.BufferGeometry) {
    ...
    // Explode geometry into objects, i.e. sides of the cube.
    var pieces = explodeGeometry(object3D.geometry, object3D.material);
    object3D.add(pieces);

    // Hide colored sides of the cube, leaving only the white edges. Must be
    // done after exploding the pieces, else they will be invisible too.
    object3D.material.visible = false;

    // Animate the sides to create the explosion effect.
    var duration = 3000;
    ...
    function explodeGeometry(geometry, material) {
